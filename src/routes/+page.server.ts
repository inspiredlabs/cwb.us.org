...
export const load: PageServerLoad = async () => {
  ...
  async function setMetaTitle() {
    ...
    contentKeywords = completion.data.choices[0].text.substring(2); // trim space
  ...
  async function setMetaDescription() {
    const completion = await openai.createCompletion({
      ...
      prompt: `Using only full, complete scentences and using the word children rather than people, continue without phrases like: "this article discusses" or "CWB stands for", or "CWB means", and continue without adding html tags, continue with plain text only, never change the CWB acronym, and continue to KEEP IT SHORT write a 19 word summary for a second-grader about CWB (Clowns Without Borders) from this article: ${messageValue}.`,
      // usage: only one-or-theother: `top_p: 1.0,` or:
      temperature: 0.3,
    ...
    contentSummary = completion.data.choices[0].text.substring(1); // trim space
